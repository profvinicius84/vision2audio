...
- 2026-07-04 popup UX follow-up: description popup, automatic close after TTS, stop-audio button, and history popup with close/clear were implemented. Core tests, Android build, and Release APK publish pass with existing AUSBC Android 16 page-size warnings.
- 2026-07-04 splash follow-up: MAUI splash screen uses root `logo.jpeg` linked as `Resources\Splash\splash.jpeg`; Release APK publish passes and generated splash resources are present. Debug build can be blocked by Visual Studio Remote Debugger file locks.
- 2026-07-04 splash conflict fix: old default `.NET` `Resources\Splash\splash.svg` is excluded from MAUI splash processing and `dotnet_bot.png` excluded from packaged images; Release APK inspection shows logo-based splash outputs only.
- 2026-07-04 OTG 3.6.0 evaluation: archive provided source but no matching `libausbc` AAR, so binding remains on current `libausbc-release.aar` plus 3.2.9 dependencies. Source analysis showed AUSBC `CaptureImage` requires raw frame queue in OpenGL mode; `CameraRequest` now enables raw preview and raw capture flags. Builds/tests/publish pass; physical OTG retest pending.
- 2026-07-04 OTG storage follow-up: `CameraUVC.captureImageInternal` uses `FileOutputStream` on supplied path; capture now prefers probed app-private external Pictures/cache paths and does not add broad storage permissions. Builds/tests/publish pass; physical OTG retest pending.
